...
using Lamps = array<array<int, n_lamps>, n_lamps>;
using Ls = Lamps::size_type;

// Functions that work on the lamps
using Func = int(*)(Lamps&, Ls, Ls);
int turn_on(Lamps &lamps, Ls x, Ls y){ return lamps[y][x] += 1; }
int turn_off(Lamps &lamps, Ls x, Ls y){ return lamps[y][x] = max(lamps[y][x] - 1, 0); }
int toggle(Lamps &lamps, Ls x, Ls y) { return lamps[y][x] += 2; }
int sum(Lamps &lamps, Ls x, Ls y){ return lamps[y][x]; }
...
int main() {
    ...
    cout << apply(sum, lamps, 0, 0, n_lamps, n_lamps) << endl;
}
