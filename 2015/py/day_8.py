...
"""
import re
print 1, sum(len(line[:-1]) - len(re.sub(r'(\\"|\\x[0-9abcdef]{2}|\\\\)', '"', line[1:-2])) for line in open('input/day_8.txt', 'rb'))
print 2, sum(2 + len(re.sub(r'("|\\)', 'XX', line)) - len(line) for line in open('input/day_8_example.txt', 'rb'))
