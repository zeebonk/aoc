...
for line in open('input/day_6.txt'):
	op, x1, y1, x2, y2 = re.match("(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)", line).groups()
	if op == 'turn on':
		matrix[int(x1):int(x2) + 1, int(y1):int(y2) + 1] = True
	elif op == 'turn of':
		matrix[int(x1):int(x2) + 1, int(y1):int(y2) + 1] = False
	else:
		matrix[int(x1):int(x2) + 1, int(y1):int(y2) + 1] = numpy.invert(matrix[int(x1):int(x2) + 1, int(y1):int(y2) + 1])
print matrix.sum()
