...
fn main() {
    ...
    let mut input_slice = input.trim_right();

    let mut count: usize = 0;

    loop {
        let result = re.captures(input_slice);
        ...
        let len: usize = cap.at(1).unwrap().parse().unwrap();
        let rep: usize = cap.at(2).unwrap().parse().unwrap();
        let (cap_start, cap_end): (usize, usize) = cap.pos(0).unwrap();

        count += cap_start + len * rep;

        input_slice = &input_slice[cap_end + len..];
    }
